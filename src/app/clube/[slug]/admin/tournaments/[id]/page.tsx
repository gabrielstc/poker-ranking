...
export default function ClubAdminTournamentDetailsPage() {
    ...
    const canManageTournaments = session && tournament && (
        session.user.role === 'SUPER_ADMIN' || 
        (session.user.role === 'CLUB_ADMIN' && session.user.clubId === tournament.clubId)
    )
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const [tournamentResponse, playersResponse] = await Promise.all([
                    ...
                    session ? fetch("/api/players") : Promise.resolve(null)
                ])
                ...
                if (tournamentResponse.ok) {
                    ...
                    // Verificar se o torneio pertence ao clube atual
                    if (currentClub && tournamentData.clubId !== currentClub.id) {
                        router.push(`/clube/${clubSlug}/admin/tournaments`)
                        return
                    }
                } else {
                    router.push(`/clube/${clubSlug}/admin/tournaments`)
        ...
        fetchData()
    }, [tournamentId, router, clubSlug, session, currentClub])

    useEffect(() => {
        if (session && tournament && !canManageTournaments) {
            router.push(`/clube/${clubSlug}`)
        }
    }, [session, tournament, canManageTournaments, router, clubSlug])
    ...
    const handleAddNewPlayer = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setSelectedPlayerName(`${newPlayer.name} (@${newPlayer.nickname})`)
                setNewPlayerData({ name: "", nickname: "", email: "", phone: "" })
                setShowAddPlayerForm(false)
                setSearchTerm("")
