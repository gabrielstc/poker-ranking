...
import { Calendar, Plus, Edit, Trash, Users, Trophy, ArrowLeft } from "lucide-react"
...
export default function ClubTournamentsPage() {
    ...
    // Verificar se o usuário tem permissão para gerenciar torneios deste clube
    const canManageTournaments = session && (
        session.user.role === 'SUPER_ADMIN' || 
        (session.user.role === 'CLUB_ADMIN' && currentClub && session.user.clubId === currentClub.id)
    )

    // Remover redirecionamento para login - permitir acesso público para visualização
    ...
    const fetchTournaments = useCallback(async () => {
        ...
        try {
            setLoading(true)
            // Buscar torneios específicos do clube
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                <Link href={`/clube/${clubSlug}`} className="hover:text-foreground flex items-center space-x-1">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold">Torneios - {currentClub.name}</h1>
                    <p className="text-muted-foreground">
                        {canManageTournaments 
                            ? `Gerencie os torneios do ${currentClub.name}`
                            : `Visualize os torneios do ${currentClub.name}`
                        }
                    </p>
                </div>
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-sm text-muted-foreground/70">
                                {canManageTournaments 
                                    ...
                                    : "Este clube ainda não possui torneios cadastrados"
                                }
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <TableRow key={tournament.id}>
                                        ...
                                        <TableCell>
                                            <div className="flex space-x-2">
                                                <Link href={`/clube/${clubSlug}/tournaments/${tournament.id}`}>
                                                    <Button size="sm" variant="default">
                                                        <Users className="h-4 w-4 mr-1" />
                                                        {canManageTournaments ? "Participantes" : "Ver Detalhes"}
                                                    </Button>
                                                </Link>
                                                {canManageTournaments && (
                                                    <>
